...
#### [The `CreatesApplication` Trait](#the-creates-application-trait)
...
```shell	
php artisanmake:testUserTest--unit
```
If you would like to create a [Pest PHP](https://pestphp.com) test, you may provide the `--pest` option to the `make:test` command:

```shell	
php artisanmake:testUserTest--pest
php artisanmake:testUserTest--unit--pest
```
> **Note**  
>  Test stubs may be customized using [stub publishing](/docs/master/artisan#stub-customization).
...
```shell	
php artisantest--testsuite=Feature--stop-on-failure
```
### [Running Tests In Parallel](#running-tests-in-parallel)
...
```shell	
php artisantest--parallel--recreate-databases
```
...
```shell	
php artisantest--coverage--min=80.3
```
### [Profiling Tests](#profiling-tests)
